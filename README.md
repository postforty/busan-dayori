...
**釜山だより (부산 다요리)**는 부산에서 나고 자란 한국인이 **일본어를 공부하고 실전 작문 실력을 키우기 위해 직접 일본어로 글을 쓰는 학습 일기**이자, 부산 현지인의 찐 맛집·골목길 정보와 함께 일본인 독자에게 실전 한국어를 알려주는 **감성형 로컬 아카이빙 & 양방향 언어 교류 플랫폼**입니다.

- **운영자 가치**: 좋아하는 부산의 이야기를 일본어로 직접 작문하고, 원어민 독자의 자연스러운 뉘앙스 교정(첨삭)을 통해 일본어 실력을 향상합니다.
- **독자(일본인 여행자) 가치**: 관광지 위주의 뻔한 정보가 아닌 현지인의 숨은 명소 정보와 함께, 여행에서 바로 쓰이는 **한글 간판 해독, 원터치 회화, 부산 사투리**를 쉽게 배웁니다.
- **상호 호혜적 언어 교류 (Win-Win)**: 한국인 운영자는 일본어를 배우고, 일본인 독자는 한국어를 배우며 서로에게 도움과 피드백을 주는 따뜻한 배움터입니다.
...
### 3. 🇰🇷 일본인 독자를 위한 한국어 학습 허브 (韓国語・ハングル学習ハブ)
- **ハングル読み (한글 읽기 마스터 3단계 스튜디오)**:
  1. **소리와 자모 조합**: 자음 14자 + 모음 10자를 실시간으로 결합해보고 Web Speech API 기반 한국어 음성 발음 재생.
  2. **획순 따라쓰기**: 캔버스 손글씨 인터랙션을 통한 한글 자모 획순 연습.
  3. **실전 간판·메뉴판 퀴즈**: 돼지국밥, 밀면, 김밥, 화장실, 출구 등 현지 여행에서 마주치는 간판을 맞히는 퀴즈 & 콤보 이펙트.
- **指差し会話 (원터치 손가락 회화)**:
  - 식당, 카페, 택시, 계산대에서 화면을 보여주기만 해도 통하는 카드.
  - 한국어 음성(TTS) 재생 지원으로 듣고 따라하기 가능.
- **釜山方言ノート (부산 사투리 사전)**:
  - 부산 골목과 시장에서 들리는 정겨운 사투리와 표준어 비교 및 현지 뉘앙스 해설.
...
│   │   ├── dialects/             # 부산 사투리 사전 페이지 (-> /korean 통합 리다이렉트)
│   │   ├── korean/               # 한국어·한글 학습 허브 (한글 읽기, 손가락 회화, 사투리)
│   │   ├── letters/              # 편지 상세, 작성(/new), 수정(/[id]/edit)
│   │   ├── phrases/              # 손가락 회화 페이지 (-> /korean 통합 리다이렉트)
│   │   ├── qa/                   # 로컬 질문방 페이지
